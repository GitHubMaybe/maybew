		...
		<script type="text/javascript">
		  ...
		  function getGrapAll(ptype){
			...
			var uId=$("#uId").val();
			  $.ajax({
				  type:"post",
				  url:"meetingpub/findAllByGrap?ptype="+ptype+"&uid="+uId,
				  success:function(msg){
					  ...
					  for(var i=0;i<msg.length;i++){
							...
							"<div style='width: 30%;float: right;'>" ;
						  	...
						  	if("2"==roleId){
						  		appendHtml+="<button class='able-btn'  onclick='grabInfo(\""+msg[i].pid+"\",\""+uId+"\")'>点击抢单</button>";	
						  	}
							appendHtml+="</div></div>";
					  } 
		  ...
		  function grabInfo(pid,uid){
			  window.location.href="meetinggrap/addGrap?pid="+pid+"&uid="+uid;
		   ...
		   function loadAjaxSelect(){
		    	 ...
		    	 $.ajax({
		    		 ...
		    		 success:function(msg){		    			 
					   var appendHtml="<option value='点击抢单' selected=true >点击抢单</option>";
		    			 for(var i=0;i<msg.length;i++){
			...
			function showMyMeetings() {
				...
				$("#one_line").css("border-top", "5px solid #4E90C7");
					...
					$.ajax({
						type:"post",
						url:"meetinggrap/findByUid",
						data:{"uid":uId},
						
						success:function(msg){
							var appendHtml = "<font style='padding: 10px 10px 10px 15px;display: block;color: #777777;'>您抢了"+msg.length+"单</font>";
							 var ptitle="";
							 var premark="";
							 var ptype="";
							 var gstatus="";
							 for(var i=0;i<msg.length;i++){	 
								 ...
								 appendHtml += "<div style='width: 100%;background-color: white;margin-top: -3px;padding:10px 10px 10px 15px;display: inline-block;'>" +
									...
									ptitle + "</div>" +
									"<div style='white-space: nowrap;overflow: hidden;text-overflow:ellipsis;display: block;color: #777777;font-size:16px;padding-top:1px'>" +
									premark + " / " + ptype + "</div></div>" +								
									"<div style='width: 30%;float: right;'>" ;
									if(gstatus==0){
										appendHtml+="<button class='able-btn' >审核中</button>"
									}else if(gstatus==1){
										appendHtml+="<button class='able-btn' >抢单失败</button>"
									}else if(gstatus==2){
